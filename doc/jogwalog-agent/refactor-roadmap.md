...
5. species picker와 date picker modal을 컴포넌트로 분리한다. 진행 중: 날짜 선택 상태와 플랫폼별 picker 실행 로직을 `hooks/use-catch-register-fishing-date.ts`로 이동했고, 어종 검색/선택 상태를 `hooks/use-catch-register-species-picker.ts`로 이동했다.
